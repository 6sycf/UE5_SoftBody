...
基于 **Unreal Engine 5.6** 的软体形变模拟项目。物理求解采用 **XPBD（Extended Position Based Dynamics）**，支持 **CPU / GPU 双路径**、**高/低模代理 LOD**，并集成了 **Geomagic OpenHaptics** 触觉力反馈设备的双向耦合交互。
...
- **触觉交互**：集成 Geomagic OpenHaptics，1000Hz 线程读取硬件，双向耦合（软体推笔、笔反馈力）。
...
- **Geomagic OpenHaptics** 触觉设备（可选，仅触觉交互需要）
- 已启用插件：`ModelingToolsEditorMode`（仅编辑器）、`GeometryScripting`、`SoftBodyGPU`

> OpenHaptics 的头文件与库已在 `ThirdParty/OpenHaptics` 内，并通过 `SoftBody.Build.cs` 自动链接，无需额外配置。
...
**触觉交互（可选）**：在场景中放置带 **`UOpenHapticsComponent`** 的 Actor，软体组件会在 `BeginPlay` 自动查找并绑定；勾选软体组件的 **Enable Haptic Interaction** 即可用触觉笔戳软体。
...
GPU 求解在 `Plugins/SoftBodyGPU` 中实现，`SoftBodyCompute.usf` 包含约 17 个 Kernel，主流程：

```
IntegrateCS（重力积分）
  → ClearLambdasCS
  → 迭代 { SolveAndApplyCS（距离约束 Jacobi）/ SolveDihedralCS（二面角）/ UpdateLambdasCS }
  → CollideStickCS（触觉笔）/ CollideGroundCS（地面）
  → UpdateVelocityCS
